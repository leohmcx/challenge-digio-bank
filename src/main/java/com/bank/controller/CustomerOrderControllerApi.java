...
public interface CustomerOrderControllerApi {
    ...
    @Operation(summary = "Retornar uma lista das compras ordenadas de forma crescente por valor, deve conter o <br/> " +
            "nome dos clientes, cpf dos clientes, dado dos produtos, quantidade das compras e valores totais de cada compra.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Compras obtidas"),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Maior compra obtidas"),
            @ApiResponse(responseCode = "204", description = "Maior compra não encontradas"),
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content)
    })
    ResponseEntity<OrderCustomerTotal> findGreatestBy(@PathVariable Integer anoCompra);
